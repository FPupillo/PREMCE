...
fit_HMM<-function(data, cBound, betaBound,initialPs){
  ...
  LB<-c( cBound[1], betaBound[1]) # lower boundary
  UB<-c(cBound[2], betaBound[2])
  # Similar to my suggestion about the number of starting points, you should add a variable to "06.Parameter_estimation" 
  # to determine the boundaries
  
  obfunc<-function(x) lik_HMM(data, x[1], 0.1, x[2],  1, initialPs) # this function is similar to the MATLAB "handle" function
