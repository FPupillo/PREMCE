...
fit_RescorlaWagner_LRchoice<-function(data,betaBound, etaBound, kBound, initialQ){
  ...
  X0<-c( rexp(1,1),runif(1, min=-10, max = 10 ), runif(1, -20, 20))  
  LB<-c(betaBound[1], etaBound[1], kBound[1]) # lower boundary
  UB<-c( betaBound[2],etaBound[2], kBound[2]) 
  # Similar to my suggestion about the number of starting points, you should add a variable to "06.Parameter_estimation" 
  # to determine the boundaries
  
  obfunc<-function(x) lik_RescorlaWagner_LRchoice(Data = data, beta = x[1],eta =  x[2], k = x[3], 1, initialQ) # this function is similar to the MATLAB "handle" function
