...
fit_Bayesian2<-function(data,betaBound, hazardBound, initialQ){
  ...
  #    hazardBound<- a two-element vector with upper and lower boundaries for the hazardRate parameter 
  #    initialQ<- inital Q values
  # Output:
  #   A list with: 
  #   [[1]] "alphabetaPAR" : alpha [1], hazardRate [2]parameters that minimize the negative log-likelihood
  ...
  X0<-c(rexp(1,1), runif(1))  # rexp generates random numbers from the exponential distributon with mean 1
  LB<-c(betaBound[1],hazardBound[1]) # lower boundary
  UB<-c(betaBound[2], hazardBound[2]) 
  # Similar to my suggestion about the number of starting points, you should add a variable to "06.Parameter_estimation" 
  # to determine the boundaries
  
  obfunc<-function(x) lik_Bayesian2(data, x[1], x[2],print=1, initialQ) # this function is similar to the MATLAB "handle" function
