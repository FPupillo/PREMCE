#------------------------------------------------------------------------------#
# Data simulation for the obs 
# created: Mon Oct 25 17:49:46 2021"
#
...
simPerf<-as.data.frame(matrix(NA, ncol = 4, nrow = sims*length(alpha)
                              *length(beta)))
names(simPerf)<-c("part", "alpha", "beta", "perOptim")

# simulate data    
pb<-txtProgressBar(min=0, max=sims*length(alpha) *length(beta))

for (simul in 1:sims){
for (a in alpha){
  for (b in beta){
  sim<-simulate_RescorlaWagner_obsALL(Data=day2list,alpha=a, beta =b,
                                    initialQ = 0.25)
...
ggplot(sim, aes(x = trialN , y =perOptimalChoice, group = alpha, colour = alpha)) +
  geom_vline(xintercept = c(48, 96,144))+
