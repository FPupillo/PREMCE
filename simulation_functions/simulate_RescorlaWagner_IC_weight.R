simulate_RescorlaWagner_IC_weight<-function ( Data,alpha, beta,  initialV){
  #----------------------------------------------------------------------------#
  # This function simulate participants' choices using a RW instructive-
  ...
  for (t in 1: nrow(Data)){
    ...
    if (count[Murkcounter]==0){
      ...
      p<- rep(initialV, 4) # if it is the first time that butterfly is shown, the Qs are at their initial value
      } else{
      ...
      p<-Data[Data$cuedCharacter==Data$cuedCharacter[t],][count[Murkcounter],Pindex] # if it is not the first time that butterfly is shown, retrieve the Qs of the last trial of that butterfly
