#------------------------------------------------------------------------------#
# Data simulation for the RW_IC_weight
# created: "Fri Dec  3 19:13:22 2021""
#
# 
#
#------------------------------------------------------------------------------#
...
source("simulation_functions/simulate_RescorlaWagner_IC_weight.R")
...
#for (a in alpha){
  for (b in beta){
    ...
    sim<-simulate_RescorlaWagner_IC_weight(Data=Data,
                                        alpha =1/Ntrial , beta = 10,  initialV = 0.25)
...
# save the data
save(list = c("simPerf", "simData"), file = "output_files/simulationData_RW_IC_weight.Rdata")
...
ggsave("output_files/simulatedData_RW_IC.jpg")
  
# now create the before/after change point graphs
AccData<-simData  %>%
  group_by(part, alpha, beta,switch_cond)  %>%
  mutate(befAft=c(rep("afterCP", times=24), rep("beforeCP", times=24)))


ggplot(AccData[1:10 & AccData$beta<10,], aes(befAft, isoptimal))+
...
ggsave("figures/optimalperf_RW_IC.jpg", plotOpt)
  

# now for only one simulation
sim<-simulate_RescorlaWagner_IC(Data=Data,
                                    alpha =0.25 , beta = 10,  initialV = 0.25)
...
ggsave("figures/estimatedCP_RW_IC_alpha=0.25_beta=10.jpg")
