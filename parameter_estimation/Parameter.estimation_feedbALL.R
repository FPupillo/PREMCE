...
for (j in 1: length(participants)){
 ...
 # DataSub<-DataSub[DataSub$listN!=0,]
  ...
  est<-fit_RescorlaWagner_feedbALL(DataSub, alphaBound,betaBound, initialQ)
  ...
  par<-lik_RescorlaWagner_feedbALL(DataSub, alpha, beta, 3, initialQ)
  ...
  if (!exists("Datalong")){
    Datalong<-lik_RescorlaWagner_feedbALL(DataSub, alpha, beta, 3, initialQ)
  } else{
    Datalong<-rbind(Datalong,lik_RescorlaWagner_feedbALL(DataSub, alpha, beta, 3, initialQ)) 
  }
...
# save parameters
write.csv(Parameters, "output_files/estimated_parameters_feedbALL.csv", row.names = F)

# save data from reinforcement learning
write.csv (Datalong, "output_files/RLdata_feedbALL.csv", row.names = F)
