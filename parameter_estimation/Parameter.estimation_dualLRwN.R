...
source("computational_model/fitting_functions/fit_dualLRwN.R")
...
for (j in 1: length(participants)){
 ...
 # DataSub<-DataAll[DataAll$SubNum==participants[j],]
  ...
  par<-lik_dualLRwN(DataSub, 0.7, 0.1, 5, 20, 3, initialQ)
  ...
  if (!exists("Datalong")){
    Datalong<-lik_dualLRwN(DataSub, 0.5, 0.1, 5, 20, 3, initialQ)
  } else{
    Datalong<-rbind(Datalong,lik_dualLRwN(DataSub, 0.7, 0.1, 5, 10, 3, initialQ)) 
  }
  
  # prepare results for saving
  #Parameters[j, c(1:5)]<-c(DataSub$SubNum[1],
  #                         0.7, 1, est$BIC, est$logLikel)
...
# save parameters
write.csv(Parameters, "output_files/estimated_parameters_dualLRwN.csv", row.names = F)

# save data from reinforcement learning
write.csv (Datalong, "output_files/RLdata_dualLRwN.csv", row.names = F)
...
# 
# for (c in c("left_categ", "centleft_categ", "right_categ", "centright_categ")){
#   par[[c]]<-as.character(par[[c]])
# }
# 
# par$character<-par$cuedCharacter
# par<-getcorrCat(par)
# 
# par$corrCat<-paste0("stimuli/", par$corr_category, ".png")
# 
# par<-getCorrkey(par)
# 
...
# 
# ggplot(par[par$trial_cond==1,], aes(x=trialN))+
#   geom_line(aes(y=CPP), size = 1.5, color = "green")+
#   facet_wrap(.~character)+
#   geom_vline(xintercept = c(48, 96,144))
