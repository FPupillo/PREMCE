...
for (j in 1: length(participants)){
 ...
 # DataSub<-DataSub[DataSub$listN!=0,]
  ...
  est<-fit_RescorlaWagner_EC(DataSub, alphaBound,betaBound, initialQ)
  ...
  par<-lik_RescorlaWagner_EC(DataSub, alpha, beta,2, initialQ)
  ...
  if (!exists("Datalong")){
    Datalong<-lik_RescorlaWagner_EC(DataSub, alpha, beta, 2, initialQ)
  } else{
    Datalong<-rbind(Datalong,lik_RescorlaWagner_EC(DataSub, alpha, beta, 2, initialQ)) 
  }
...
# save parameters
write.csv(Parameters, "output_files/estimated_parameters_RW_EC.csv", row.names = F)

# save data from reinforcement learning
write.csv (Datalong, "output_files/RLdata_RW_EC.csv", row.names = F)
