...
for (j in 1: length(participants)){
 ...
 # DataSub<-DataSub[DataSub$listN!=0,]
  ...
  if (!exists("Datalong")){
    Datalong<-lik_RescorlaWagner_IC(DataSub, alpha, beta, 2, initialQ)
  } else{
    Datalong<-rbind(Datalong,lik_RescorlaWagner_IC(DataSub, alpha, beta, 2, initialQ)) 
  }
...
# save parameters
write.csv(Parameters, "output_files/estimated_parameters_RW_IC.csv", row.names = F)
