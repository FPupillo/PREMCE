...
colnames(Parameters)<-c("PartNum", "c","gamma", "BIC", "LL") #names of the columns
...
for (j in 1: length(participants)){
 ...
 # DataSub<-DataSub[DataSub$listN!=0,]
  ...
  est<-fit_HMM(DataSub, cBound,gammaBound, initialQ)
  ...
  par<-lik_HMM(DataSub, c, gamma,2, initialQ)
  ...
  if (!exists("Datalong")){
    Datalong<-lik_HMM(DataSub, c, gamma,2, initialQ)
  } else{
    Datalong<-rbind(Datalong,lik_HMM(DataSub, c, gamma,2, initialQ)) 
  }
  
  # prepare results for saving
  Parameters[j, c(1:5)]<-c(DataSub$participant[1],
                           c, gamma, est$BIC, est$logLikel)
...
# save parameters
write.csv(Parameters, "output_files/estimated_parameters_HMM.csv", row.names = F)

# save data from reinforcement learning
write.csv (Datalong, "output_files/RLdata_HMM.csv", row.names = F)
